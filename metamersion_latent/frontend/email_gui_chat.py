...
def get_key(pg_keycode):
    ...
    if pygame.key.get_mods() & pygame.KMOD_SHIFT:
        ...
        if pg_keycode == pygame.K_AT:
            return "@"
        
        elif pg_keycode >= pygame.K_a and pg_keycode <= pygame.K_z:
...
class ChatGUI:
    ...
    def init_chat_session(self, username="NONE"):
        ...
        self.dp_out = os.path.join(
            "../../data/chats", f"{get_time('second')}_{username}"
        )
        ...
        if self.send_message:
            ...
            if self.send_message_timer == 0:
                ...
                if time.time() > self.time_start + self.config.initial_chat_time_limit:
                    ...
                    self.first_text_human = deepcopy(self.last_text_human)
                    output = self.ask_for_email()
                    # output = self.chat(self.last_text_human)
                elif self.message_count == 2:
                    print(self.first_text_human)
                    self.set_email()
                    output = "Thanks. " + self.chat(self.first_text_human)
                else:
                    output = self.chat(self.last_text_human)
                output = output.strip()
                print(f"GOT: {output}")
    ...
    def ask_for_email(self):
        output = "Can you give me your email? I will try to send you a video later about our conversation."
        self.chat_active = True
        return output
    
    def set_email(self):
        self.email = self.last_text_human
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fp_config", type=str, default="../configs/chat/ls1_version_5_watts.py")
    ...
    parser.add_argument("--run_fullscreen", type=bool, default=False)
